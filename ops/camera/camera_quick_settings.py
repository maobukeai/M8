...
class M8_OT_CameraQuickSettings(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        if hasattr(cam, "show_limits"):
            box = layout.box()
            box.label(text=_T("视图显示"), icon="RESTRICT_VIEW_OFF")
            col = box.column(align=True)
            col.prop(cam, "show_limits", text=_T("显示范围"))
            col.prop(cam, "show_mist", text=_T("显示雾场"))
            col.prop(cam, "show_sensor", text=_T("显示传感器"))
            col.prop(cam, "show_name", text=_T("显示名称"))
            col.prop(cam, "show_composition_center", text=_T("显示中心点"))
            
        if hasattr(cam, "dof"):
            box = layout.box()
            box.label(text=_T("景深"), icon="CAMERA_DATA")
            dof = cam.dof
