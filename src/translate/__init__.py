            ...
            if language not in all_language:
                if language in ("zh_CN", "zh_HANS"):
                    if "zh_CN" in all_language:
                        ...
                        language = "zh_HANS"
            if language in data:
                data[language].update(value)
            else:
                data[language] = value

    for language, value in data.items():
        try:
            ti = TranslationHelper(f"M8_{language}", value, lang=language)
            ti.register()
            __language_list__.append(ti)
            print(f"M8 Align: Translation registered for {language}")
        except Exception as e:
            print(f"M8 Align: Failed to register translation for {language}: {e}")
