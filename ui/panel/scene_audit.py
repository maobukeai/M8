...
class VIEW3D_PT_M8_SceneAudit(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(wm_state, "scene_audit_high_poly_threshold", text="高面数")
        row = layout.row(align=True)
        row.prop(wm_state, "scene_audit_make_backup", text="备份")
        row.prop(wm_state, "scene_audit_backup_collection_name", text="")

        row = layout.row(align=True)
        op = row.operator("m8.run_scene_audit", text="选中")
        op.scope = "SELECTED"
        op.high_poly_threshold = wm_state.scene_audit_high_poly_threshold
        op = row.operator("m8.run_scene_audit", text="可见")
        op.scope = "VISIBLE"
        op.high_poly_threshold = wm_state.scene_audit_high_poly_threshold
        op = row.operator("m8.run_scene_audit", text="场景")
        op.scope = "ALL"
        op.high_poly_threshold = wm_state.scene_audit_high_poly_threshold

        row = layout.row(align=True)
        row.operator("m8.select_scene_audit_problem_objects", text="选择问题", icon="RESTRICT_SELECT_OFF")
        ...
        op = fix_row.operator("m8.fix_scene_audit_safe_issues", text="安全修复", icon="BRUSH_DATA")
        op.scope = last_scope
        ...
        op = fix_row.operator("m8.select_scene_audit_issue_objects", text="安全几何")
        op.scope = last_scope
        op.issue_filter = "SAFE_GEOMETRY"

        row = layout.row(align=True)
        op = row.operator("m8.select_scene_audit_issue_objects", text="拓扑")
        op.scope = last_scope
        op.issue_filter = "TOPOLOGY"
        op = row.operator("m8.select_scene_audit_issue_objects", text="材质")
        op.scope = last_scope
        op.issue_filter = "MATERIALS"
        op = row.operator("m8.select_scene_audit_issue_objects", text="变换")
        op.scope = last_scope
        op.issue_filter = "TRANSFORMS"
        op = row.operator("m8.select_scene_audit_issue_objects", text="高面数")
        op.scope = last_scope
        op.issue_filter = "HIGH_POLY"

        backup_row = layout.row(align=True)
        op = backup_row.operator("m8.select_scene_audit_backups", text="备份", icon="PRESET")
        op.backup_collection_name = wm_state.scene_audit_backup_collection_name
        op.reveal = True
        op = backup_row.operator("m8.restore_scene_audit_selected_backups", text="恢复选中")
        op.rescan = True
        op.delete_backups = False
        op = backup_row.operator("m8.restore_scene_audit_latest_backups", text="恢复最新")
        op.backup_collection_name = wm_state.scene_audit_backup_collection_name
        op.rescan = True
        op.delete_backups = False

        manage_row = layout.row(align=True)
        op = manage_row.operator("m8.refresh_scene_audit_backups", text="刷新")
        op.backup_collection_name = wm_state.scene_audit_backup_collection_name
        manage_row.operator("m8.copy_scene_audit_backup_report", text="", icon="COPYDOWN")
        manage_row.prop(wm_state, "scene_audit_backup_keep_per_source", text="保留")
        op = manage_row.operator("m8.prune_scene_audit_backups", text="清理")
        op.backup_collection_name = wm_state.scene_audit_backup_collection_name
        ...
        if fix_history:
            ...
            history_box.label(text="审计历史", icon="PRESET")
