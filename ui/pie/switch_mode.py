...
def edit_mesh(context: bpy.types.Context, pie: bpy.types.UILayout):
    ...
    row.prop(tool, "use_proportional_edit", text=_T("衰减"), toggle=True, icon="PROP_CON")
    ...
    # Snap
    row = col.row(align=True)
    row.prop(tool, "use_snap", text=_T("吸附"), toggle=True, icon="SNAP_ON")
    sub = row.row(align=True)
    ...
    column = pie.column(align=True)
    column.scale_y = 1.3
    column.operator(M8_OT_MeshOptimizationMenu.bl_idname, icon="MOD_REMESH", text=_T("优化"))
    if hasattr(tool, "use_mesh_automerge"):
        column.prop(tool, "use_mesh_automerge", text=_T("自动合并顶点"))
    column.operator(
        "view3d.toggle_xray",
        icon="XRAY",
        text=(_T("遮挡") if is_xray else _T("穿透")),
        depress=is_xray
    )
...
def draw_switch_view_3d_mode_operator(context: bpy.types.Context, pie: bpy.types.UILayout, pref, direction: str):
    ...
    if mode == "SWITCH_MODE":
        ...
        text = (_T("编辑模式") if is_object else _T("物体模式"))
    ...
    label_map = {"VERT": _T("顶点"), "EDGE": _T("边"), "FACE": _T("面")}
...
def draw_switch_image_mode_operator(context: bpy.types.Context, pie: bpy.types.UILayout, pref, direction: str, override_mode: str = None):
    ...
    if mode == "SWITCH_MODE":
        ...
        if context.area.ui_type == 'UV' or context.area.type == 'IMAGE_EDITOR':
            text = (_T("物体模式") if is_edit_mesh else _T("编辑UV"))
        else:
            text = (_T("物体模式") if is_edit_mesh else _T("编辑模式"))
...
def draw_switch_bone_mode_operator(context: bpy.types.Context, pie: bpy.types.UILayout, pref, direction: str):
    ...
    if mode == "EDIT_OR_OBJECT":
        text, icon, mode = (
            (_T("物体模式"), "OBJECT_DATAMODE", "OBJECT")
            if om == "EDIT" or om not in ("EDIT", "OBJECT") else
            (_T("编辑模式"), "EDITMODE_HLT", "EDIT")
        )
        ...
        if om == "POSE":
            ops = pie.operator("object.mode_set", text=_T("编辑模式"), icon="EDITMODE_HLT")
            ops.mode = "EDIT"
            ops.toggle = True
        else:
            pie.separator()
        ...
        text, icon, value = (_T("切换到静置"), "POSE_HLT", "REST") if is_pose else (
            _T("切换到姿态"), "ARMATURE_DATA", "POSE",)
        ops = pie.operator("wm.context_set_enum", text=text, icon=icon)
        ops.data_path = "object.data.pose_position"
        ops.value = value
    elif mode == "VIEW_SELECTED":
        pie.operator("view3d.view_selected", text=_T("聚焦"), icon="VIEWZOOM")
    elif mode == "TOGGLE_XRAY":
        op = pie.operator("wm.context_toggle", text=_T("前台显示"), icon="XRAY", depress=active.show_in_front)
        op.data_path = "active_object.show_in_front"
    elif mode == "TOGGLE_NAMES":
        op = pie.operator("wm.context_toggle", text=_T("显示名称"), icon="SORTALPHA", depress=data.show_names)
        op.data_path = "object.data.show_names"
    elif mode == "TOGGLE_AXES":
        op = pie.operator("wm.context_toggle", text=_T("显示轴向"), icon="AXIS_SIDE", depress=data.show_axes)
...
def draw_common_view_ops(pie):
    pie.operator("view3d.view_selected", text=_T("聚焦"), icon="VIEWZOOM")
    pie.operator("view3d.localview", text=_T("局部视图"), icon="HIDE_OFF")


def draw_extrude_bevel_props(pie, data):
    if hasattr(data, "extrude"):
        pie.prop(data, "extrude", text=_T("挤出"))
    else:
        pie.separator()
    if hasattr(data, "bevel_depth"):
        pie.prop(data, "bevel_depth", text=_T("倒角"))
    else:
        pie.separator()
...
def draw_node_editor_ops(context: bpy.types.Context, pie: bpy.types.UILayout):
    ...
    col = pie.column(align=True)
    col.scale_y = 1.3
    col.operator("node.view_selected", text=_T("聚焦"), icon="VIEWZOOM")
    col.separator()
    col.separator()
    ...
    col = pie.column(align=True)
    col.scale_y = 1.3
    col.operator("node.mute_toggle", text=_T("禁用/启用"), icon="CHECKBOX_DEHLT")
    col.operator("node.join", text=_T("加入框"), icon="NODE_SEL")
    col.operator("node.parent_clear", text=_T("移出框"), icon="UNLINKED")

    # 3. Bottom
    pie.operator("node.join", text=_T("加入框"), icon="NODE_SEL")
    ...
    if is_group_selected:
        pie.operator("node.group_edit", text=_T("编辑组"), icon="NODETREE")
    elif is_inside_group:
        pie.operator("node.tree_path_parent", text=_T("退出组"), icon="FILE_PARENT")
    else:
        pie.operator("node.group_edit", text=_T("编辑组"), icon="NODETREE")

    # 5. Top Left
    pie.operator("node.group_make", text=_T("成组"), icon="NODETREE")

    # 6. Top Right
    pie.operator("node.group_ungroup", text=_T("解组"), icon="X")

    # 7. Bottom Left
    col = pie.column(align=True)
    col.scale_y = 1.3
    col.operator("wm.call_menu", text=_T("搜索添加"), icon="ADD").name = "NODE_MT_add"
    col.operator("node.group_make", text=_T("打组"), icon="NODE_MATERIAL")

    # 8. Bottom Right
    col = pie.column(align=True)
    col.scale_y = 1.3
    col.operator("node.view_all", text=_T("查看全部"), icon="VIEWZOOM")
    col.operator("node.backimage_fit", text=_T("重置节点"), icon=_ICON("LOOP_BACK"))
...
class VIEW3D_MT_M8SwitchModePie(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        if obj.type == "LIGHT":
            light = obj.data
            pie.prop(light, "energy", text=_T("能量"))

            if getattr(light, "type", "") == "SUN" and hasattr(light, "angle"):
                pie.prop(light, "angle", text=_T("角度"))
            elif getattr(light, "type", "") in {"POINT", "SPOT"} and hasattr(light, "shadow_soft_size"):
                pie.prop(light, "shadow_soft_size", text=_T("半径"))
            elif getattr(light, "type", "") == "AREA" and hasattr(light, "size"):
                pie.prop(light, "size", text=_T("尺寸"))
            else:
                pie.separator()

            pie.operator(M8_OT_LightTrackToSelected.bl_idname, text=_T("对准选中"), icon="CON_TRACKTO")
            pie.operator(M8_OT_LightQuickSettings.bl_idname, text=_T("灯光设置"), icon="LIGHT_DATA")

            if hasattr(light, "use_shadow"):
                op = pie.operator("wm.context_toggle", text=_T("投影"), depress=light.use_shadow)
                op.data_path = "object.data.use_shadow"
            else:
                pie.separator()

            draw_common_view_ops(pie)
            pie.operator(M8_OT_LightClearTrackTo.bl_idname, text=_T("清除对准"), icon="X")
            return

        if obj.type == "CAMERA":
            cam = obj.data
            pie.prop(cam, "lens", text=_T("焦距"))

            try:
                pie.prop(context.scene.view_settings, "exposure", text=_T("曝光"))
            except Exception:
                pie.separator()

            pie.operator(M8_OT_CameraFocusSelected.bl_idname, text=_T("对焦到选中"), icon="TRACKING_FORWARDS")
            pie.operator(M8_OT_CameraQuickSettings.bl_idname, text=_T("相机设置"), icon="PREFERENCES")

            pie.operator("view3d.view_camera", text=_T("查看摄像机"), icon="CAMERA_DATA")

            if context.space_data and context.space_data.type == 'VIEW_3D':
                is_locked = context.space_data.lock_camera
                op = pie.operator("wm.context_toggle", text=_T("锁定相机到视图"), icon="LOCKVIEW_ON", depress=is_locked)
                op.data_path = "space_data.lock_camera"
            else:
                pie.separator()

            pie.operator(M8_OT_SelectSceneCamera.bl_idname, text=_T("设为活动摄像机"), icon="OUTLINER_OB_CAMERA")
            pie.operator("view3d.localview", text=_T("局部视图"), icon="HIDE_OFF")
            return
        ...
        if obj.type == "LATTICE":
            ...
            # 1. Left (4)
            pie.operator("view3d.view_selected", text=_T("聚焦 4"), icon="VIEWZOOM")

            # 2. Right (6)
            pie.operator(M8_OT_LatticeMakeRegular.bl_idname, text=_T("重置形状 6"), icon="MESH_GRID")

            # 3. Bottom (2)
            col = pie.column(align=True)
            col.scale_y = 1.2
            col.prop(data, "interpolation_type_u", text=_T("U 插值"))
            col.prop(data, "interpolation_type_v", text=_T("V 插值"))
            col.prop(data, "interpolation_type_w", text=_T("W 插值"))

            # 4. Top (8)
            ops = pie.operator(M8_OT_ModeSetRemember.bl_idname, text=(_T("编辑模式 8") if is_object else _T("物体模式 8")), icon=("EDITMODE_HLT" if is_object else "OBJECT_DATAMODE"))
            ...
            if hasattr(data, "use_outside"):
                op = pie.operator("wm.context_toggle", text=_T("外部影响"), icon="MOD_LATTICE", depress=data.use_outside)
        ...
        if obj.type == "FONT":
            ...
            if context.mode == "EDIT_TEXT":
                pie.operator("wm.call_menu_pie", text=_T("编辑工具 4"), icon="TOOL_SETTINGS").name = "VIEW3D_MT_m8_text_edit_pie"
            else:
                pie.separator()

            # 2. Right
            pie.operator(M8_OT_TextQuickSettings.bl_idname, text=_T("文字设置 6"), icon="FONT_DATA")

            # 3. Bottom
            pie.operator("wm.call_menu_pie", text=_T("转换 2"), icon="MESH_DATA").name = "VIEW3D_MT_m8_curve_convert_pie"

            # 4. Top (Original Left - Toggle Mode)
            ops = pie.operator(M8_OT_ModeSetRemember.bl_idname, text=(_T("编辑文字 8") if is_object else _T("物体模式 8")), icon=("EDITMODE_HLT" if is_object else "OBJECT_DATAMODE"))
            ops.mode = ("EDIT" if is_object else "OBJECT")
        ...
        if obj.type == "META":
            ...
            # 这里放阈值属性比较合适
            pie.prop(data, "threshold", text=_T("阈值 6"))
            ...
            # 融球特有的分辨率设置
            col = pie.column(align=True)
            col.prop(data, "resolution", text=_T("视图分辨率"))
            col.prop(data, "render_resolution", text=_T("渲染分辨率"))
            return
        ...
        if obj.type in {"CURVE", "SURFACE"}:
            ...
            if context.mode == "EDIT_CURVE":
                pie.operator("wm.call_menu_pie", text=_T("编辑工具 4"), icon="TOOL_SETTINGS").name = "VIEW3D_MT_m8_curve_edit_pie"
            else:
                pie.operator(M8_OT_CurveCyclicToggle.bl_idname, text=_T("切换闭合 4"), icon="RECOVER_LAST")

            # 2. Right
            pie.operator(M8_OT_CurveQuickSettings.bl_idname, text=_T("曲线设置 6"), icon="CURVE_DATA")
        ...
        if obj.type == "CURVES":
            ...
            pie.operator("view3d.view_selected", text=_T("聚焦"), icon="VIEWZOOM")
            # 2. Right
            op = pie.operator(M8_OT_ObjectConvertToMesh.bl_idname, text=_T("转网格(保留)"), icon="MESH_DATA")
            op.keep_original = True
            # 3. Bottom
            op = pie.operator(M8_OT_ObjectConvertToMesh.bl_idname, text=_T("转网格(替换)"), icon="OUTLINER_OB_MESH")
            op.keep_original = False
            # 4. Top (Toggle)
            pie.operator("object.mode_set", text=(_T("雕刻曲线") if is_object else _T("物体模式")), icon=("SCULPTMODE_HLT" if is_object else "OBJECT_DATAMODE")).mode = ("SCULPT_CURVES" if is_object else "OBJECT")
        ...
        if obj.type == "ARMATURE":
            ...
            pie.operator(
                ...
                text=(_T("物体模式 6") if is_pose else _T("姿态模式 6")),
            ...
            col = pie.column(align=True)
            col.scale_y = 1.2
            col.prop(data, "display_type", text=_T("显示样式"))
            if is_pose or is_edit:
                row = col.row(align=True)
                row.prop(data, "show_bone_custom_shapes", text=_T("自定义形状"), icon="BONE_DATA")

            # 4. Top (8)
            ops = pie.operator(
                "object.mode_set",
                text=(_T("物体模式 8") if is_edit else _T("编辑模式 8")),
            ...
            ops = pie.operator("wm.context_set_enum", text=(_T("切换到姿态") if is_rest else _T("切换到静置")), icon=("POSE_HLT" if is_rest else "ARMATURE_DATA"))
            ops.data_path = "object.data.pose_position"
            ...
            col = pie.column(align=True)
            col.scale_y = 1.3
            op = col.operator("wm.context_toggle", text=_T("前台显示"), icon="XRAY", depress=obj.show_in_front)
            op.data_path = "object.show_in_front"
            op = col.operator("wm.context_toggle", text=_T("显示名称"), icon="SORTALPHA", depress=data.show_names)
            op.data_path = "object.data.show_names"
            op = col.operator("wm.context_toggle", text=_T("显示轴向"), icon="AXIS_SIDE", depress=data.show_axes)
            op.data_path = "object.data.show_axes"
            ...
            if is_edit or is_pose:
                col.prop(data, "use_mirror_x", text=_T("X轴对称"), icon="MOD_MIRROR")
            else:
                pie.separator()
        ...
        if obj.type == "EMPTY":
            ...
            # 2. Right
            col = pie.column(align=True)
            col.prop(obj, "empty_display_size", text=_T("尺寸"))
            if is_image:
                col.prop(obj, "empty_image_offset", text=_T("偏移"))

            # 3. Bottom
            pie.operator("wm.call_menu_pie", text=_T("切换类型"), icon="EMPTY_DATA").name = "VIEW3D_MT_m8_empty_type_pie"

            # 4. Top
            op = pie.operator("wm.context_toggle", text=_T("前台显示"), icon="XRAY", depress=obj.show_in_front)
            op.data_path = "object.show_in_front"
            ...
            if is_image:
                col = pie.column(align=True)
                col.prop(obj, "empty_image_depth", text=_T("深度"))
                col.prop(obj, "empty_image_side", text=_T("侧面"))
            else:
                pie.separator()

            # 7. Bottom Left
            col = pie.column(align=True)
            col.prop(obj, "show_name", text=_T("显示名称"), icon="SORTALPHA")
            col.prop(obj, "show_bounds", text=_T("显示边界"), icon="BBOX")
            ...
            if is_image:
                ...
                row = col.row(align=True)
                row.prop(obj, "use_empty_image_alpha", text=_T("透明度"), icon="RESTRICT_VIEW_OFF")
                if obj.use_empty_image_alpha:
                     row.prop(obj, "color", index=3, text="", slider=True)

                row = col.row(align=True)
                row.prop(obj, "show_empty_image_orthographic", text=_T("正交"), toggle=True)
                row.prop(obj, "show_empty_image_perspective", text=_T("透视"), toggle=True)
            else:
                pie.separator()
        ...
        if obj.type in {"GPENCIL", "GREASEPENCIL"}:
            ...
            # 1. Left (4)
            ops = pie.operator("object.mode_set", text=(_T("绘制模式 4") if not is_draw else _T("物体模式 4")), icon=("GREASEPENCIL" if not is_draw else "OBJECT_DATAMODE"))
            ops.mode = ("PAINT_GPENCIL" if not is_draw else "OBJECT") if obj.type == "GPENCIL" else ("PAINT_GREASEPENCIL" if not is_draw else "OBJECT")

            # 2. Right (6)
            ops = pie.operator("object.mode_set", text=(_T("雕刻模式 6") if not is_sculpt else _T("物体模式 6")), icon=("SCULPTMODE_HLT" if not is_sculpt else "OBJECT_DATAMODE"))
            ...
            col = pie.column(align=True)
            col.scale_y = 1.2
            ops = col.operator("object.mode_set", text=(_T("顶点绘制") if not is_vertex else _T("物体模式")), icon=("VPAINT_HLT" if not is_vertex else "OBJECT_DATAMODE"))
            ...
            ops = col.operator("object.mode_set", text=(_T("权重绘制") if not is_weight else _T("物体模式")), icon=("WPAINT_HLT" if not is_weight else "OBJECT_DATAMODE"))
            ...
            # 4. Top (8)
            ops = pie.operator("object.mode_set", text=(_T("编辑模式 8") if not is_edit else _T("物体模式 8")), icon=("EDITMODE_HLT" if not is_edit else "OBJECT_DATAMODE"))
            ...
            if hasattr(data, "use_autolock_layers"):
                op = pie.operator("wm.context_toggle", text=_T("自动锁定图层"), icon="LOCKED", depress=data.use_autolock_layers)
            ...
            if has_depth or has_front:
                ...
                if has_front:
                    col.prop(obj, "show_in_front", text=_T("前台显示"), icon="XRAY")
            else:
                pie.separator()
            ...
            if has_onion or has_curve:
                ...
                col.scale_y = 1.3
                if has_onion:
                    op = col.operator("wm.context_toggle", text=_T("洋葱皮"), icon="ONIONSKIN_ON", depress=data.use_onion_skinning)
                    op.data_path = "object.data.use_onion_skinning"
                if has_curve:
                    op = col.operator("wm.context_toggle", text=_T("多边形曲线"), icon="CURVE_BEZCURVE", depress=data.use_stroke_edit_curve)
        ...
        if obj.type == "POINTCLOUD":
            ...
            # 4. Top (8)
            ops = pie.operator("object.mode_set", text=(_T("编辑模式 8") if is_object else _T("物体模式 8")), icon=("EDITMODE_HLT" if is_object else "OBJECT_DATAMODE"))
            ops.mode = "EDIT" if is_object else "OBJECT"
        ...
        # 4. Top (Toggle)
        is_object = context.mode == "OBJECT"
        pie.operator("object.mode_set", text=(_T("编辑模式") if is_object else _T("物体模式")), icon=("EDITMODE_HLT" if is_object else "OBJECT_DATAMODE")).mode = ("EDIT" if is_object else "OBJECT")
...
class VIEW3D_MT_M8CurveEditPie(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        ops = pie.operator(M8_OT_CurveHandleTypeRemember.bl_idname, text=_T("手柄:Auto"), icon="HANDLE_AUTO", depress=(last_handle == "AUTOMATIC"))
        ops.handle_type = "AUTOMATIC"
        ops = pie.operator(M8_OT_CurveHandleTypeRemember.bl_idname, text=_T("手柄:Align"), icon="HANDLE_ALIGNED", depress=(last_handle == "ALIGNED"))
        ops.handle_type = "ALIGNED"
        ops = pie.operator(M8_OT_CurveHandleTypeRemember.bl_idname, text=_T("手柄:Vector"), icon="HANDLE_VECTOR", depress=(last_handle == "VECTOR"))
        ops.handle_type = "VECTOR"
        ops = pie.operator(M8_OT_CurveHandleTypeRemember.bl_idname, text=_T("手柄:Free"), icon="HANDLE_FREE", depress=(last_handle == "FREE_ALIGN"))
        ops.handle_type = "FREE_ALIGN"

        pie.operator(M8_OT_CurveSwitchDirectionRemember.bl_idname, text=_T("反转方向"), icon="FILE_REFRESH", depress=(last_action == "SWITCH_DIRECTION"))
        ops = pie.operator(M8_OT_CurveSubdivideRemember.bl_idname, text=_T("细分"), icon="SUBDIVIDE_EDGES", depress=(last_action == "SUBDIVIDE"))
        ops.number_cuts = 1
        pie.operator(M8_OT_CurveSmoothRemember.bl_idname, text=_T("平滑"), icon="SMOOTHCURVE", depress=(last_action == "SMOOTH"))
        pie.operator(M8_OT_CurveCyclicToggle.bl_idname, text=_T("切换闭合"), icon="RECOVER_LAST")
...
class VIEW3D_MT_M8EmptyTypePie(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        # 1. Left - 十字
        pie.prop_enum(obj, "empty_display_type", "PLAIN_AXES", text=_T("十字"), icon="EMPTY_AXIS")
        # 2. Right - 箭头
        pie.prop_enum(obj, "empty_display_type", "ARROWS", text=_T("箭头"), icon="EMPTY_ARROWS")
        # 3. Bottom - 单箭头
        pie.prop_enum(obj, "empty_display_type", "SINGLE_ARROW", text=_T("单箭头"), icon="EMPTY_SINGLE_ARROW")
        # 4. Top - 图片
        pie.prop_enum(obj, "empty_display_type", "IMAGE", text=_T("图片"), icon="IMAGE_DATA")
        # 5. Top Left - 立方体
        pie.prop_enum(obj, "empty_display_type", "CUBE", text=_T("立方体"), icon="CUBE")
        # 6. Top Right - 球体
        pie.prop_enum(obj, "empty_display_type", "SPHERE", text=_T("球体"), icon="SPHERE")
        # 7. Bottom Left - 锥体
        pie.prop_enum(obj, "empty_display_type", "CONE", text=_T("锥体"), icon="CONE")
        # 8. Bottom Right - 圆环
        pie.prop_enum(obj, "empty_display_type", "CIRCLE", text=_T("圆环"), icon="MESH_CIRCLE")
...
class VIEW3D_MT_M8TextEditPie(bpy.types.Menu):
    ...
    def draw(self, context):
        pie = self.layout.menu_pie()
        pie.operator("font.text_copy", text=_T("复制"), icon="COPYDOWN")
        pie.operator("font.text_paste", text=_T("粘贴"), icon="PASTEDOWN")
        pie.operator("font.text_cut", text=_T("剪切"), icon="CUT")
        pie.operator("font.delete", text=_T("删除"), icon="X")
        pie.operator("font.select_all", text=_T("全选"), icon="SELECT_SET")
        pie.operator("font.line_break", text=_T("换行"), icon="SORTBYEXT")
        pie.separator()
        pie.separator()
